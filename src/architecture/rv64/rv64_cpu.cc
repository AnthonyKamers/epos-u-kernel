...
{
    // take the previous stack pointer (which was put in sscratch previously), to save it in the context later
    ASM("       csrr     x3,  sscratch          \n" // sscratch = usp (preserving user stack pointer across system call)
        "       sd       x3,    0(a0)           \n" // push usp (to kernel stack, to restore later)
        "       sd       x1,    8(a0)           \n");
